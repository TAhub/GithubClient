...
class GithubService
{
	...
	class func fetchUsers(searchTerm:String, completion:(String?, [User]?)->())
	{
		do
		{
			let token = try OAuthClient.shared.accessToken()
			let urlString = "https://api.github.com/search/users?q=\(searchTerm)&access_token=\(token)"
			...
			if let URL = NSURL(string: urlString)
			{
				...
				session.dataTaskWithRequest(request, completionHandler:
					...
					else if let data = data, let users = GithubJSONParser.usersFromNSData(data)
					{
						NSOperationQueue.mainQueue().addOperationWithBlock()
						{
